...
function encodeRelativeURL(startPath, relative, callerPath) {
  ...
  if (fs.existsSync(absStart)) {
    var fullPath = path.resolve(startPath, relative);
    ...
    // recurse paths down and up except the caller path
      ...
      // subdirectories of the start path
      var subDirectories = fs.readdirSync(absStart)
      ...
      // parent of the start path
      var parent = [ path.resolve(absStart, '..') ]
        .filter(function limitToWorkingDirectory(absoluteDir) {
          return (path.relative(process.cwd(), absoluteDir).indexOf('..') < 0);
        });

      // look deeper first
      subDirectories.concat(parent)
...
module.exports = function (bannerWidth, libraryPaths) {
  ...
  return through.obj(function (file, encoding, done) {
     ...
     * This functions gets called with an object containing a CSS file and its sourcemap,
     * which is modified and passed through CSS rework, before being pushed to the results.
     * @param {{css: string, map: string, stats: object}} results Results object from node-sass
     * @param {string} map The source-map for the compiled css
     */
    function successHandler(results) {
      var css = results.css;
      var map = convert.fromComment(css).toObject();

      // adjust source-map
      map.sources.forEach(function (value, i, array) {
        array[i] = path.resolve(value.replace(/^\//, ''));  // ensure absolute
      });
      var cssAdjusted = css.replace(convert.commentRegex, '/* ' + convert.fromObject(map).toComment(css) + ' */');

      // prepare the adjusted sass source-map for later look-ups
      sourceMapConsumer = new SourceMapConsumer(map);

      // rework css
      var reworked = rework(cssAdjusted, '')
        .use(reworkPlugin)
        .toString({
          sourcemap        : true,
     ...
     * This function simply constructs a printable error message string,
     * and pushes it to the output array.
     *
     * @param {object} error The error object from node-sass
     */
    function errorHandler(error) {
      var message = error.file + ':' + error.line + ':' + error.column + ' - ' + error.message;
      output.push(message);
      done();
    }
